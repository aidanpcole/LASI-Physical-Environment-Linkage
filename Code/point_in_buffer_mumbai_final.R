...
lasi_coor <- subset(lasi_coor, stateid %in% c("27.Maharashtra"))
...
physenv <- st_read("C:/Users/Aidan/Desktop/mumbai-combined-objects.geojson") %>%
  st_drop_geometry()
...
#   summarize shop info
###############################################################################
# Output is "long" by buffer (hhid), one row for each shop
...
join5[nrow(join5) + 1,] <- list(as.character(127179500310100),NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,miss_buffer$geometry)


st_write(mumbai_quarts, "C:/Users/Aidan/Desktop/dummygeoboy/mumbai_quarts.geojson")

mumbai_quarts <- join5 %>%
  mutate(per_na = num_na/num_ints,
         quart_na = ntile(per_na,4))

mumbai_plz_work <- join5 %>%
  st_drop_geometry()

write.csv(join5, "C:/Users/Aidan/Desktop/mumbai_w_phys.csv")
write.csv(mumbai_plz_work, "C:/Users/Aidan/Desktop/mumbai_phys.csv")

st_write(mumbai_map, "C:/Users/Aidan/Desktop/dummygeoboy/mumbai_quants.geojson")


mumbai_map <- join5 %>%
  mutate(per_na = num_na/num_ints) %>%
  mutate(quart_na = ntile(per_na,4))

### graphs and figures 
mumbai_final <- join5 
hist(mumbai_final$num_ints)
...
hist(mumbai_final$num_na)
hist(mumbai_final$people_gt)
hist(mumbai_final$cars_gt)
hist(mumbai_final$gvi_per)
hist(mumbai_final$img_ins)
hist(mumbai_final$img_seg)
hist(mumbai_final$road_per)


mumbai.sf <- mumbai_final%>%
  mutate(hhid = as.numeric(mumbai_final$hhid)) %>%
...
numericVars <-
  select_if(st_drop_geometry(mumbai.sf), is.numeric) %>% na.omit()
